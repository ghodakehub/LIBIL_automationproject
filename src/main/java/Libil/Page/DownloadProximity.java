...
public class DownloadProximity extends BasePage {

	public DownloadProximity(WebDriver driver) {
		super(driver);
	}
	...
	public boolean downloadJSON(String keyword) throws InterruptedException {
        ...
        driver.findElement(By.xpath("//label[contains(text(),'Proximity ')]")).click();
        Thread.sleep(2000);
        
        // Wait for the download button to be visible
        WebElement downloadButton = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[contains(text(),'Download Proximity')]")));
        downloadButton.click();

        // Verify if the JSON file is downloaded
        boolean isDownloaded = waitForDownloadAndValidate("data", ".json", Duration.ofMinutes(4));

        if (isDownloaded) {
            System.out.println("JSON file downloaded successfully for: ");
	            ...
	            for (File file : files) {
	                ...
	                if (file.getName().startsWith(fileNamePrefix) && file.getName().endsWith(extension) && file.length() > 0) {
	                    ...
	                    try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
	                        String line;
	                        int lineCount = 0;
	                        System.out.println("JSON Content (First 3 lines):");
